"""
shapes_sizes_experiment - hypercommon vs 4 baselines on rings of differing SIZE.

The companion sweep (shapes_algos_experiment) held ring size uniform and varied
the degree z from ring to ring. This one does the opposite: within a shape every
ring has the SAME z, but the rings differ in how many nodes they contain, so a
single graph holds communities at several scales.

That matters because modularity-based methods carry a resolution limit - small
communities below a size that depends on the total edge count get absorbed into
their neighbours - while hypercommon's triple criterion is local and has no
notion of graph-wide scale. Ring size is exactly the axis that should expose the
difference.

Holding z fixed makes the comparison unusually clean: edges = n * z / 2 no matter
how the sizes are split, so every shape at a given z has an identical edge count,
identical mean degree, and identical cost. Only the partition changes.
...
Output - results/shapes_sizes/run_<ts>/
...
ring_size holds the FIRST ring's size here, since sizes differ within a shape;
the full list is in the sizes column of shapes.csv. Everything else a plot might
key on is on the row itself. This script only computes and writes; plotting is a
separate step over records.csv.

Run from venv:
  ./.venv/Scripts/python.exe -m experiments.ring_lattice.shapes_sizes_experiment
  ./.venv/Scripts/python.exe -m experiments.ring_lattice.shapes_sizes_experiment --validate --build
  HC_ONLY_SHAPES=sz_50x20_100x10_z8 ./.venv/Scripts/python.exe -m experiments.ring_lattice.shapes_sizes_experiment
"""
...
# Ordered MOST EXPENSIVE FIRST. Shapes are launched as concurrent processes and
# each walks its own units serially, so the sweep finishes when the slowest shape
# does. Starting the z=32 shapes first lets the cheap z=8 ones fill in alongside
# them instead of leaving a long tail at the end.
#
# Within a shape every ring carries the same z; the rings differ in SIZE. Because
# z is fixed, edges = n * z / 2 regardless of the split, so shapes sharing a z
# have identical edge counts and identical cost - the partition is the only thing
# that changes, which is what makes the comparison clean.
#
# Shapes come in z=8 / z=16 pairs over the same size pattern, so the size effect
# can be checked at two densities; the z=32 pair adds a denser regime.
#
# Constraints: every ring needs z < its own size, and the edge count must be
# divisible by 100 for p to reach exactly 1.0.
#
...
SHAPES: list[tuple[str, list[int], list[int]]] = [
    # --- mean degree 32: ~63 h each, the long pole ---
    ("sz_ladder_z32",
     [100] * 4 + [200] * 3 + [300] * 2 + [400],
     [32] * 10),
    ("sz_100x10_200x5_z32",  [100] * 10 + [200] * 5, [32] * 15),

    # --- mean degree 16: ~12 h each ---
    ("sz_50x20_100x10_z16",  [50] * 20 + [100] * 10, [16] * 30),
    ("sz_50x20_200x5_z16",   [50] * 20 + [200] * 5,  [16] * 25),
    ("sz_mixed4_z16",
     [50] * 5 + [100] * 5 + [200] * 5 + [250],
     [16] * 16),

    # --- mean degree 8: ~2.4 h each ---
    ("sz_50x20_100x10_z8",   [50] * 20 + [100] * 10, [8] * 30),
    ("sz_50x20_200x5_z8",    [50] * 20 + [200] * 5,  [8] * 25),
    ("sz_mixed4_z8",
     [50] * 5 + [100] * 5 + [200] * 5 + [250],
     [8] * 16),
]
...
OUTPUT_ROOT = os.path.join("results", "shapes_sizes")
...
def check_shape(sizes: list[int], zs: list[int]) -> list[str]:
    ...
    if len(set(sizes)) < 2:
        problems.append("ring sizes do not vary - this sweep varies SIZE, so a "
                        "uniform-size shape belongs in shapes_algos_experiment")

    if len(set(zs)) != 1:
        problems.append(f"z is not uniform across rings: {sorted(set(zs))} - varying z "
                        "is the companion sweep, shapes_algos_experiment")
...
def run(out_root: str | None = None) -> None:
    ...
    log("=== shapes_sizes_experiment start ===")
    ...
    log(f"=== shapes_sizes_experiment done ===  total dt={time.perf_counter() - started:.0f}s")
