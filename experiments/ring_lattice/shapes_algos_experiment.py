"""
shapes_algos_experiment — hypercommon vs 6 baselines on rings of differing degree.

The completed n=2000 runs (results/overlap_algos/) both used ONE degree for every
ring. This sweep breaks that: ring size stays uniform, but z varies from ring to
ring, so the communities in a single graph differ in density.

That matters because hypercommon applies one global threshold while the best
threshold depends on degree — t* ~ 0.177 at z=8 against ~ 0.118 at z=16, measured
on the uniform corpus. Rings of differing z each "want" a different t, and only
one can be chosen. The baselines have no threshold at all, so any cost this
imposes falls on hypercommon alone; the comparison is a fair test rather than a
favourable one.
...
For each (shape, overlap%, run):
  ...
  3. At every p, run all seven algorithms on the same graph snapshot, in
     parallel, and score each against the ground truth with the omega index.
...
Output — results/shapes_algos/run_<ts>/
...
Everything a plot might key on is on the row itself — no joins needed to draw
omega against p, against overlap, or against any shape property. This script
only computes and writes; plotting is a separate step over records.csv.

Run from venv:
  ./.venv/Scripts/python.exe -m experiments.ring_lattice.shapes_algos_experiment
  ./.venv/Scripts/python.exe -m experiments.ring_lattice.shapes_algos_experiment --validate
  ./.venv/Scripts/python.exe -m experiments.ring_lattice.shapes_algos_experiment --validate --build
  HC_ONLY_SHAPES=n2000_r50x40_5level ./.venv/Scripts/python.exe -m experiments.ring_lattice.shapes_algos_experiment
  HC_WORKERS=10 ./.venv/Scripts/python.exe -m experiments.ring_lattice.shapes_algos_experiment
"""
...
# n = sum(sizes) and the ring count is len(sizes). Every shape holds ring SIZE
# uniform and varies only z across rings.
#
#   A/B    ring size 50  (40 rings) — the small-community arm
#   C/D/E  ring size 200 (10 rings) — the large-community arm
#
# A, B and E all sit at mean degree 16.0, matching the completed
# n=2000 z=16 ring=100 run, so they are comparable to it and to each other; only
# ring size and the shape of the z spread differ.
#
# Uniform-z references already exist and are not repeated here:
#   results/overlap_algos/n=2000_z=16_r=100   all 7 algos, 6 overlaps, 10 runs
#   results/overlap_algos/n=2000_z=8_r=50     all 7 algos, 6 overlaps, 10 runs
#   results/pcrit_experiment/final_result     t_best and p_crit, n=400..3000
#
# z is capped at 40. Cost grows as roughly mean_degree**2.35, and past that a
# ring also stops looking like a sparse community: at ring size 200, z=64 gives a
# ring density of 0.32, dense enough that any algorithm finds it and the result
# says more about density than about heterogeneity.
#
# Names appear in output paths and in the shape column of every results row, so
# keep them short, unique and stable.
SHAPES: list[tuple[str, list[int], list[int]]] = [
    # A — five z levels spread evenly over 40 small rings
    ("n2000_r50x40_5level",
     [50] * 40,
     [8] * 8 + [12] * 8 + [16] * 8 + [20] * 8 + [24] * 8),

    # B — same ring size and mean degree as A, but only two extremes.
    # A vs B isolates the SHAPE of the spread from its magnitude.
    ("n2000_r50x40_extreme",
     [50] * 40,
     [8] * 20 + [24] * 20),

    # C — five z levels over 10 large rings
    ("n2000_r200x10_5level",
     [200] * 10,
     [8] * 2 + [16] * 2 + [24] * 2 + [32] * 2 + [40] * 2),

    # D — the two-extreme counterpart of C, at ring size 200
    ("n2000_r200x10_extreme",
     [200] * 10,
     [8] * 5 + [32] * 5),

    # E — skewed: many sparse rings, few dense ones (40% of nodes at z=8,
    # 10% at z=32). Closer to how degree is distributed in real networks, and a
    # direct test of whether one global threshold follows the majority of nodes
    # and abandons the dense minority.
    ("n2000_r200x10_skewed",
     [200] * 10,
     [8] * 4 + [16] * 3 + [24] * 2 + [32] * 1),
]
...
OUTPUT_ROOT = os.path.join("results", "shapes_algos")
...
def check_shape(sizes: list[int], zs: list[int]) -> list[str]:
    ...
    if len(set(sizes)) != 1:
        problems.append(f"ring sizes are not uniform: {sorted(set(sizes))}")

    if len(set(zs)) < 2:
        problems.append("z does not vary across rings — already covered by the "
                        "uniform runs under results/overlap_algos")
...
def run(out_root: str | None = None) -> None:
    ...
    log("=== shapes_algos_experiment start ===")
    ...
    log(f"=== shapes_algos_experiment done ===  total dt={time.perf_counter() - started:.0f}s")
